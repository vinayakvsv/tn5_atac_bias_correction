...
#
make_counts_matrix <- function(seqbias_counts,plyrmethod=FALSE,
                               ...) {
  # #make the counts matrix. Old solution
  # sb.lens <- as.data.frame(t(as.data.frame(lapply(X = seqbias_counts,length)))) #THE ERROR IS HERE
  # rownames(sb.lens) <- names(seqbias_counts)
  # mode.len <- names(sort(-table(sb.lens)))[1]
  # remove <- rownames(sb.lens)[!sb.lens == mode.len]
  # seqbias_counts <- seqbias_counts[!seqbias_counts %in% remove]
  ...
  if (plyrmethod) {
    print("WARNING: plyrmethod HAS A BUG! INTERPRET RESULTS WITH CAUTION!")
    sb.counts <- as.data.frame(ldply(.data = seqbias_counts,.fun = rbind,.id = NULL,na.rm=TRUE))
    sb.counts.sum <- colSums(x = sb.counts,na.rm = TRUE)
    #THE BUG: THE COVERAGE TRACK APPEARS TO BE HALF OF WHAT THE ORIGINAL COUNTS MATRIX WAS.
  } else {
    #GET THE LENGTHS OF THE MOTIF INSTANCES
    sb.lens <- as.data.frame(t(as.data.frame(lapply(X = seqbias_counts,length)))) #THE ERROR IS HERE
    rownames(sb.lens) <- names(seqbias_counts)
    #IDENTIFY THE 'MODE' LENGTH
    mode.len <- names(sort(-table(sb.lens)))[1]
    remove <- rownames(sb.lens)[!sb.lens == mode.len]
    #print(length(seqbias_counts))
    #REMOVE THE INTERVALS THAT HAVE A LENGTH NOT IN THE MODE
    seqbias_counts <- seqbias_counts[!names(seqbias_counts) %in% remove]
    #print(length(seqbias_counts))
    #NOW MAKE A COUNTS MATRIX
    sb.counts <- as.data.frame(t(as.data.frame(lapply(X = seqbias_counts,unlist))))
    sb.counts.sum <- apply(X = sb.counts,MARGIN = 2,sum)
  }
...
pos_neg_counts <- function(sb.poscounts,
  ...
  poscounts <- make_counts_matrix(sb.poscounts) #uneven lengths occur here. Need to remove them...
  negcounts <- make_counts_matrix(sb.negcounts) #uneven lengths occur here. Need to remove them...
...
seqbias_before <- function(ref_seq,
  ...
  sb.ppm.pos.before <- make_pwm(freqs = seqbias.inputs.pos$freqs,name = paste(name,"pos",sep="_")) #THERE IS AN ERROR HERE AFFECTING SOME OF THE MOTIFS...
  ...
  sb.ppm.neg.before <- make_pwm(freqs = seqbias.inputs.neg$freqs,name = paste(name,"neg",sep="_")) #THERE IS AN ERROR HERE AFFECTING SOME OF THE MOTIFS...
...
seqbias_after <- function(seqbias.inputs.pos,
  ...
  seqbias.corrected.pos <- estimate_bias(sb=posmodel,
                                         ...
                                         name = paste(name,"pos",sep="_"))
  sb.ppm.pos.after <- make_pwm(freqs = seqbias.corrected.pos$freqs,name = paste(name,"pos",sep="_"))
  ...
  seqbias.corrected.neg <- estimate_bias(sb=negmodel,
                                         ...
                                         name = paste(name,"neg",sep = "_"))
  sb.ppm.neg.after <- make_pwm(freqs = seqbias.corrected.neg$freqs,name = paste(name,"neg",sep="_"))
...
#assign the model strings
ctrlmodel.fn <- ctrlmodel
testmodel.fn <- testmodel
print(ctrlmodel.fn)
print(testmodel.fn)
#ctrlmodel.fn <- buenrostropos
#load up the files
ctrlmodel.yaml <- seqbias.load(ref_fn = ref_seq,
                                         model_fn = ctrlmodel.fn)
print(paste("Loaded","control model:",ctrlmodel.fn))
testmodel.yaml <- seqbias.load(ref_fn = ref_seq,
                               model_fn = testmodel.fn)
print(paste("Loaded","test model:",testmodel.fn))
...
#The Shendure model
print("Impose the control model")
after.ctrlmodel <- seqbias_after(seqbias.inputs.pos = before$pos$seqbias,
                                          seqbias.inputs.neg = before$neg$seqbias,
                                          posmodel = ctrlmodel.yaml,
                                          negmodel = ctrlmodel.yaml,
                                          name = paste(name,"post","ctrlmodel",sep = "_"))
#The Buenrostro ATAC-seq model
print("Impose the test model")
after.testmodel <- seqbias_after(seqbias.inputs.pos = before$pos$seqbias,
                                       seqbias.inputs.neg = before$neg$seqbias,
                                       posmodel = testmodel.yaml,
                                       negmodel = testmodel.yaml,
                                       name = paste(name,"post","testmodel",sep = "_"))
...
before.footprint <- pos_neg_counts(sb.poscounts = before$pos$seqbias$counts,
                                   ...
                                   clipbp=clip) #MAJOR SOURCES OF ERROR. SOME INSTANCES+FLANKS OF A MOTIF ARE SHORTER THAN OTHER INSTANCES+FLANKS OF THE MOTIF. POTENTIAL FIX WITH ldply AND colSums IN ORDER TO REMOVE NA's
...
#After correction with the test models
#generate the counts track
print("Generate footprints after correction with Buenrostro ATAC-seq models")
after.testmodel.footprint <- pos_neg_counts(sb.poscounts = after.testmodel$pos$seqbias$counts,
                                                  sb.negcounts = after.testmodel$neg$seqbias$counts,
                                                  motiflen = motiflen,
                                                  name = paste(name,"after_testmodel",sep="."),
                                                  clipbp=clip)
#create a spline model
after.testmodel.spline <- measure_deflection.spline(footprint = after.testmodel.footprint$cutsite.freq,
                                                     name = paste(name,"after_testmodel",sep="."),
                                                     motiflen = motiflen,
                                                     nonmotiflen = nonmotiflen)
#write the counts track to an output
write.table(x = after.testmodel.footprint$cutsite.freq,
            file = paste(name,"after_testmodel","shifted_freq","txt",sep = "."),
            quote = FALSE,sep = "\t",
            row.names = TRUE,
            col.names = TRUE)
after.testmodel.sb <- c(after.testmodel,after.testmodel.footprint,after.testmodel.spline)
saveRDS(after.testmodel.sb,file = paste(name,"after_testmodel","rds",sep = "."))

#After correction with the Shendure chr22 model
print("Generate footprints after correction with Shendure chr22 models")
after.ctrlmodel.footprint <- pos_neg_counts(sb.poscounts = after.ctrlmodel$pos$seqbias$counts,
                                                     sb.negcounts = after.ctrlmodel$neg$seqbias$counts,
                                                     motiflen = motiflen,
                                                     name = paste(name,"after_ctrlmodel",sep="."),
                                                     clipbp=clip)
#create a spline model
after.ctrlmodel.spline <- measure_deflection.spline(footprint = after.ctrlmodel.footprint$cutsite.freq,
                                                          name = paste(name,"after_ctrlmodel",sep="."),
                                                          motiflen = motiflen,
                                                          nonmotiflen = nonmotiflen)
#write the counts track to an output
write.table(x = after.ctrlmodel.footprint$cutsite.freq,
            file = paste(name,"after_ctrlmodel","shifted_freq","txt",sep = "."),
            quote = FALSE,sep = "\t",
            row.names = TRUE,
            col.names = TRUE)
after.ctrlmodel.sb <- c(after.ctrlmodel,after.ctrlmodel.footprint,after.ctrlmodel.spline)
saveRDS(after.ctrlmodel.sb,file = paste(name,"after_ctrlmodel","rds",sep = "."))
...
par(mfrow=c(1,3))
#Before correction
freqmax <- max(before.footprint$cutsite.freq$freq,
               after.ctrlmodel.footprint$cutsite.freq$freq,
               after.testmodel.footprint$cutsite.freq$freq)
freqmin <- min(before.footprint$cutsite.freq$freq,
               after.ctrlmodel.footprint$cutsite.freq$freq,
               after.testmodel.footprint$cutsite.freq$freq)
#freqlim <- 0.03
#freqlim <- c(freqmin,freqmax)
freqlim <- c(0,0.03)
...
plot(after.ctrlmodel.footprint$cutsite.freq$freq,
     xaxt="n",
     type="l",
     ylim=freqlim,
     col="blue",
...
plot(after.testmodel.footprint$cutsite.freq$freq,
     xaxt="n",
     type="l",
     ylim=freqlim,
     col="blue",
